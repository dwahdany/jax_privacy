...
def _normalize_image(
    ...
    if preprocess_name == "center":
        ...
        raise NotImplementedError(
            f"Preprocessing with {preprocess_name} not implemented for ChexPert."
        )
...
class ChexpertConfig(base.ImageDatasetConfig):
    ...
    uncertain_to_positive: Sequence[str] = ("Atelectasis", "Edema", "Pleural Effusion")
    ...
    def __post_init__(self):
        ...
        if self.patient_percentile_min != 0.0 or self.patient_percentile_max != 1.0:
            assert (
                0 <= self.patient_percentile_min <= self.patient_percentile_max <= 1.0
            )
    ...
    def select_patient(self, patient_id: tf.Tensor) -> tf.Tensor:
        ...
        permuted_patient_id = tf.cast(tf.reshape(permuted_patient_id, []), tf.float32)
...
class AbstractChexpertLoader(loader.DataLoader, metaclass=abc.ABCMeta):
    ...
    def __post_init__(self):
        if self.preprocessed_resolution not in (300, 512):
            raise ValueError(
                f"Dataset not available for {self.preprocessed_resolution} resolution."
            )
    ...
    def filter_dataset(self, ds: tf.data.Dataset) -> tf.data.Dataset:
        ...
        if (
            self.config.patient_percentile_min != 0.0
