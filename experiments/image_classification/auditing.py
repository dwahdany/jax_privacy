...
from experiments import image_data
from experiments.image_classification import evaluator
from experiments.image_classification import metrics as metrics_module
from experiments.image_data import base, loader
from jax_privacy.dp_sgd import typing
from jax_privacy.training import experiment_config, forward, updater
        ...
        match self.canary_type:
            ...
            case CanaryType.CUSTOM:
                if self.in_canary_eval is None:
                    raise ValueError(
                        "custom_canary_fn must be set for custom canaries."
                    )
            case _:
...
def data_loader_with_only_canaries(
    ...
    new_config = dataclasses.replace(data_loader.config, num_samples=len(canaries))
    return dataclasses.replace(
        data_loader,
        load_raw_data_tf_fn=new_load_raw_data_fn,
        config=new_config,
...
class CanaryGradientForwardFnWrapper(
    ...
    def __init__(
        self,
        base_forward_fn: forward.ForwardFn[image_data.DataInputs, hk.Params, hk.State],
    ):
    ...
    def eval_forward(
        ...
        return self._base_forward_fn.eval_forward(params, network_state, rng, inputs)
